# Job Search Agent V1.8.3

V1.8.3 is the **application-package integrity and cover-letter template correction** release. It keeps the V1.8.2 relevance/deep-budget logic and Hotfix-1 Windows-safe package paths, while correcting the real document-generation issues found in Alpha, Da Vinci, Akkodis and Jobtimum output.

## V1.8.3 — what changed

- Ships sanitized German and English cover-letter templates based on the preferred recent application style.
- Targets substantive role-specific cover letters (roughly 400–560 words when evidence supports it) instead of short 180–280 word notes.
- Separates factual support from broken evidence metadata: `TRACE_MISSING` and `TRACE_MISMATCH` can be repaired without falsely rejecting truthful claims.
- Audits document claims against the full verified evidence registry, so legitimate retained CV claims can recover the correct project evidence IDs.
- Performs one bounded automatic correction + re-audit for genuinely overstated/unsupported document claims.
- Validates generated PDF artifacts instead of relying only on the LaTeX process return code.
- Recovery/repair runs now populate `last_run_report.json`, so dashboard mode/usage/runtime are accurate.
- Adds `repair_packages()` / `python agent.py repair-packages` to rebuild existing needs-review packages from cached deep matches without another job search or job-matching cycle.
- Matching `analysis_version` remains `1.8.2` intentionally, so your existing V1.8.2 deep results can be reused.

### Recommended upgrade from V1.8.2 Hotfix 1

If you want to preserve your existing database and avoid repeating Codex job matching, copy the V1.8.3 code over the existing project **without deleting `output/job_agent.sqlite3`**, then run in `vscode_runner.py`:

```python
result = repair_packages()
```

That mode regenerates existing packages marked `needs_ai_or_review`. It performs no discovery, HTTP page checking, SCREEN or DEEP matching. When you later want a normal new search cycle, use:
...
See `CHANGELOG_V1_8_3.md` and `VALIDATION_REPORT.md` for details.
